...
Desde el Ministerio de ModernizaciÃ³n de la NaciÃ³n se propone la transparencia
desde el acceso abierto a los datos del Estado. Es una iniciativa muy loable.
Aunque, por ahora, son pocos datasets los que estÃ¡n disponibles. 
Espero que en el futuro aumente ya que es mucha la informaciÃ³n que se produce
en el Estado.

La plataforma [datos.gob.ar](http://www.datos.gob.ar) estÃ¡ desarrollada con 
...
sido desarrollada (rOpenSci)[ropensci.org]. EstÃ¡ disponible en
[CRAN](https://cran.r-project.org/web/packages/ckanr/index.html) y la 
versiÃ³n de desarrollo en [GitHub](https://github.com/ropensci/ckanr).
...
{% highlight r %}
## Probamos si estÃ¡ instaldo crankr y sino lo instalamos
...
Ahora vamos a probar ver que datasets estÃ¡n disponibles en datos.gob.ar. Primero
...
Probamos nuestra conexiÃ³n al servidor:
...
Ahora podemos empezar por ver que datasets estÃ¡n disponibles


{% highlight r %}
# Guardo los datasets porque despuÃ©s vamos a usar el nombre como id
...
Probablemente haya que investigar un poco mÃ¡s. La ayuda de la funciÃ³n 
...
es `package_list_current()` muestra mucha informaciÃ³n sobre los datasets. Mucha
de ella relativa al sitio web. Una mucho mÃ¡s interesante es `package_search()`
que busca tÃ©rminos en los metadatos de los datasets.
...
Y `package_show()` muestra informaciÃ³n referida a algÃºn dataset en particular.
...
Hay mÃ¡s funciones como para crear, actualizar y borrar datasets, pero para eso
hay que tener una clave.

Con organization_list() podemos ver todas instituciones del gobierno que hay
provistos datos y otra informaciÃ³n como cuantos datos y el nÃºmero de usuarios.
...
Y usando `organization_show` y el hash de una organizaciÃ³n en particular 
podemos ver la informaciÃ³n solo de esa.
...
TambiÃ©n es posible buscar algÃºn recurso en particular. Los recursos son en
general archivos con datos. Las bÃºsquedas se realizan en el formato 
...
EstÃ¡ claro que hay algunos que serÃ¡n mÃ¡s Ãºtiles que otros. Por ejemplo, para
buscar todos los recursos en formato CSV:


{% highlight r %}
# Guardamos los resultados como tabla para que sea mÃ¡s fÃ¡cil manipular
csv <- resource_search(q = "format:csv", as = "table")[[2]]
{% endhighlight %}

Por defecto, la informaciÃ³n mostrada es limitada. Pero si guardamos 
...
mÃ¡s sencillo encontrar estas cosas.
...
{% highlight r %}
# Eliminar un caracter extraÃ±o que indica el BOM (byte order mark)
names(presupuesto_2016) <- str_replace(names(presupuesto_2016), "\uFEFF", "")
{% endhighlight %}

En el presupuesto hay mucha informaciÃ³n. La informaciÃ³n se encuentra bastante 
desagregada, estoy seguro que algÃºn periodista de datos podrÃ¡ sacarles mucho
jugo. Toda la informaciÃ³n sobre los campos del archivo la pueden encontrar
[acÃ¡](https://github.com/gobabiertoAR/datasets-portal/blob/master/presupuesto/presupuesto-documentacion.md) 

Dejo algÃºnos grÃ¡ficos a modo de ejemplo.
...
presupuesto_2016 %>% 
  ...
  ggplot(presupuesto_2016_by_jurisdiccion, aes(jurisdiccion_desc, monto_presupuestado)) +
    ...
    scale_x_discrete("JurisdicciÃ³n") + 
